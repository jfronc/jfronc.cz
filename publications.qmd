...
books <- pubs |> filter(`Item Type` == "book")
...
book_items <- books |>
  mutate(
    ...
    entry = str_glue("
     - {Author}. {make_link(Title, Url, DOI)}. {Place}: {Publisher}, {Year}{make_pg(Pages)}. {make_pdf(Fulltext)} {make_bo(BO)}")
  )
...
chapter_items <- chapters |>
  mutate(
    entry = str_glue("- {Author}. *{Title}*. In: {`Book Author`}. *{make_link(`Publication Title`, Url, DOI)}*. {Place}: {Publisher}, {Year}{make_pg(Pages)}. {make_pdf(Fulltext)} {make_bo(BO)}")
  )

# Print sections
cat("### Kniha\n\n")
cat(book_items$entry, sep="\n")
